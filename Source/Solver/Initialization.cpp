        ...
        amrex::ParallelFor( bx, [=] AMREX_GPU_DEVICE (int i, int j, int k)
        {
            ...
            if (mask(i,j,k) >= 2.0) {
                ...
                if (mask(i,j,k) == 2.0) { // intrinsic
                    ...
                    // Check if we're near the junction for smooth transition
                    if (std::abs(distance_from_junction) < transition_width) {
                        // Smooth transition region
                        Real tanh_factor = std::tanh(distance_from_junction / (transition_width/3.0));
                        Real weight = 0.5 * (1.0 + tanh_factor); // 0 to 1 transition
                        
                        // N-type values (left side)
                        Real n_ntype = donor_doping;
                        Real p_ntype = intrinsic_carrier_concentration * intrinsic_carrier_concentration / donor_doping;
                        
                        // P-type values (right side)  
                        Real n_ptype = intrinsic_carrier_concentration * intrinsic_carrier_concentration / acceptor_doping;
                        Real p_ptype = acceptor_doping;
                        
                        // Smooth interpolation
                        initial_n = n_ntype * (1.0 - weight) + n_ptype * weight;
                        initial_p = p_ntype * (1.0 - weight) + p_ptype * weight;
                    } else {
                        // Normal P-type initialization far from junction
                        initial_p = acceptor_doping;
                    ...
                    if (std::abs(distance_from_junction) < transition_width) {
                        ...
                        // Normal N-type initialization far from junction
                        initial_n = donor_doping;
